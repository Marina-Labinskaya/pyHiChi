...
//OMP_FOR_COLLAPSE()
    ...
    void PeriodicalFieldGenerator<gridTypes>::generateB()
    {
        ...
        for (int dim0 = 0; dim0 < grid->dimensionality; dim0++)
        {
            int dim1 = (dim0 + 1) % 3;
            int dim2 = (dim0 + 2) % 3;
            int begin1 = 0;// this->fieldSolver->internalEAreaBegin[dim1];
            int begin2 = 0;// this->fieldSolver->internalEAreaBegin[dim2];
            int end1 = grid->numCells[dim1];// this->fieldSolver->internalEAreaEnd[dim1];
            int end2 = grid->numCells[dim2];// this->fieldSolver->internalEAreaEnd[dim2];
//OMP_FOR_COLLAPSE()
            for (int j = begin1; j < end1; j++)
                for (int k = begin2; k < end2; k++)
                {
